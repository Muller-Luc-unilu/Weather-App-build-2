...
public final class WeatherUtils {
    ...
    public List<WeatherForecast> filtertoDaytime(List<WeatherForecast> forecasts) {
        return forecasts.stream()
                .filter(f -> f.time().contains("09:00:00") || f.time().contains("12:00:00")
                        || f.time().contains("15:00:00") || f.time().contains("18:00:00")
                        || f.time().contains("21:00:00"))
                .toList();
    }
    ...
    public Map<String, List<WeatherForecast>> getMap(List<WeatherForecast> forecast,
            Map<String, List<WeatherForecast>> byDay) {
